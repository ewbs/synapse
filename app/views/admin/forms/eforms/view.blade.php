...
<div class="row">
	...
	<div class="col-md-8">
		<div class="block-flat">
			...
			<div class="content">
				<p><strong>Disponible en ligne : </strong>{{$modelInstance->disponible_en_ligne ? Eform::disponibleEnLigne()[$modelInstance->disponible_en_ligne] : Eform::disponibleEnLigne()['non_communique']}}</p>
				<p><strong>Déposable en ligne : </strong>{{$modelInstance->deposable_en_ligne ? Eform::deposableEnLigne()[$modelInstance->deposable_en_ligne] : Eform::deposableEnLigne()['non_communique']}}</p>
				<p><strong>Dématérialisation : </strong>{{$modelInstance->dematerialisation ? Eform::dematerialisation()[$modelInstance->dematerialisation] : Eform::dematerialisation()['non_communique']}} @if($modelInstance->dematerialisation == 'oui') le {{$modelInstance->getDematerialisationDate() }} @endif</p>
				<p><strong>Canal de dématérialisation : </strong>
					@if($modelInstance->dematerialisation_canal != 'autres')
						{{ $modelInstance->dematerialisation_canal ? Eform::dematerialisationCanal()[$modelInstance->dematerialisation_canal] : Eform::dematerialisationCanal()['non_communique'] }}
					@else
						{{ $modelInstance->dematerialisation_canal_autres }}
					@endif
				</p>
				<p><strong>Intervention eWBS  : </strong>{{$modelInstance->intervention_ewbs ? Eform::interventionEwbs()[$modelInstance->intervention_ewbs] : Eform::interventionEwbs()['non_communique']}}</p>
