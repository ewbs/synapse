...
class AdminRolesController extends BaseController {
	...
	protected function routeGetIndex() { return route('rolesGetIndex'); }
	...
	public function postCreate() {
		...
		if ($validator->passes ()) {
			...
			if ($this->role->id) {
				...
				return Redirect::secure ( 'admin/roles/' . $this->role->id . '/edit' )->with ( 'success', Lang::get ( 'admin/roles/messages.create.success' ) );
			}
		...
		return Redirect::secure ( 'admin/roles/create' )->withInput ()->withErrors ( $validator );
	}
	...
	public function getEdit($role) {
		if (! empty ( $role )) {
			...
			return Redirect::secure ( 'admin/roles' )->with ( 'error', Lang::get ( 'admin/roles/messages.does_not_exist' ) );
		}
	...
	public function postEdit($role) {
		...
		if ($validator->passes ()) {
			...
			if ($role->save ()) {
				...
				return Redirect::secure ( 'admin/roles/' . $role->id . '/edit' )->with ( 'success', Lang::get ( 'admin/roles/messages.update.success' ) );
			} else {
				// Redirect to the role page
				return Redirect::secure ( 'admin/roles/' . $role->id . '/edit' )->with ( 'error', Lang::get ( 'admin/roles/messages.update.error' ) );
			}
		}
		
		// Form validation failed
		return Redirect::secure ( 'admin/roles/' . $role->id . '/edit' )->withInput ()->withErrors ( $validator );
	}
	...
	public function postDelete($role) {
		...
		if ($role->delete ()) {
			...
			return Redirect::secure ( 'admin/roles' )->with ( 'success', Lang::get ( 'admin/roles/messages.delete.success' ) );
		}
		
		// There was a problem deleting the role
		return Redirect::secure ( 'admin/roles' )->with ( 'error', Lang::get ( 'admin/roles/messages.delete.error' ) );
	}
